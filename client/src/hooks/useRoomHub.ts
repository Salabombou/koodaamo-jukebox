...
import { SeededRandom } from "../services/shuffleService";
...
export default function useRoomHub() {
  ...
  // Helper function to move item in queue
  const moveItemInQueue = useCallback((from: number, to: number) => {
    ...
    if (from >= itemList.length || to >= itemList.length) return null;

    const movedItem = itemList[from];
    itemList.splice(from, 1);
    itemList.splice(to, 0, movedItem);
  ...
  useEffect(() => {
    ...
    connection.current.on("ShuffleToggled", (event: ShuffleToggledEvent) => {
      ...
      startTransition(() => {
        ...
        if (event.is_shuffled && typeof event.seed === "number") {
          ...
          const shuffledItems = [...otherItems];
          const rng = new SeededRandom(event.seed);
    ...
    connection.current.on("QueueMoved", (event: QueueMovedEvent) => {
      ...
      startTransition(() => {
        ...
        const updatedItems = moveItemInQueue(event.from, event.to);
  ...
  }, [discordSDK.isEmbedded, moveItemInQueue, isShuffled]);
